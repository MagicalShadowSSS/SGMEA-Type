...
def ckpt_for(dataset, rate, variant):
    ...
    table = {
        "FBDB15K:0.5:baseline": "SGMEA_FBDB15K_0.5_baseline_warm_fbdb_rates_cdmr_0511_104108_r05_",
        "FBDB15K:0.5:tcms": "SGMEA_FBDB15K_0.5_fbdb_r05_keep_repro_seed42_",
        "FBYG15K:0.5:baseline": "SGMEA_FBYG15K_0.5_baseline_warm_fbyg_specific_prior_0512_111257_lr5e4_r05_",
        "FBYG15K:0.5:tcms": "SGMEA_FBYG15K_0.5_fbyg_r05_tcms_keep_ckpt_",
    }
    if key not in table:
...
def build_args(base, dataset, checkpoint, use_tcms=False, use_dehr=False):
    old_argv = sys.argv
    sys.argv = [old_argv[0]]
    try:
        config = Config()
    ...
    args.data_rate = base.data_rate
    args.model_name = "SGMEA"
    ...
    args.disable_sgmea_guidance = True
    args.csls = True
    args.csls_k = 3
...
def main():
    ...
    for dataset, data in results.items():
        ...
        if not v:
            continue
        print(
            f"{dataset}\t{v['vsc_before']:.4f}\t{v['vsc_after']:.4f}\t"
    ...
    for dataset, data in results.items():
        ...
        if not r:
            continue
        print(
            f"{dataset}\tDeltaL1={r['avg_delta_l1']:.4f}\t"
