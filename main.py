...
try:
    from src.loss_upper_bound_probe import run_loss_upper_bound_probe
except ImportError:
    run_loss_upper_bound_probe = None
from src.utils import set_optim, Loss_log, pairwise_distances, csls_sim
try:
    from src.geometry_probe import run_geometry_probe
except ImportError:
    run_geometry_probe = None
...
class Runner:
    def __init__(self, args, writer=None, logger=None, rank=0):
        ...
        self.data_init()
        self.model_choise()
        self._apply_relation_hnm_lr_policy()
        set_seed(args.random_seed)
    ...
    def model_choise(self):
        ...
        if self.args.model_name == "SGMEA":
            self.model = SGMEA(self.KGs, self.args)
        ...
        if (
            self.args.model_name == "SGMEA"
            and getattr(self.args, "use_tmhg_router", False)
        ):
            self.model.init_tmhg_train_stats(self.train_ill, logger=self.logger)
        ...
        if self.args.use_relation_aware_hnm and getattr(self.model, "relation_aware_hnm_cache_stats", None) is not None:
            cache_stats = self.model.relation_aware_hnm_cache_stats
            self.logger.info(
                "RelationAwareHNMCache | "
                f"rows={cache_stats['row_count']} "
                f"ltr_anchors={cache_stats['anchor_count_by_direction'].get('left_to_right', 0)} "
                f"rtl_anchors={cache_stats['anchor_count_by_direction'].get('right_to_left', 0)} "
                f"safe={cache_stats['status_counts'].get('safe_negative', 0)} "
                f"close={cache_stats['status_counts'].get('closely_related', 0)}"
            )

    def _apply_relation_hnm_lr_policy(self):
        if self.args.only_test:
            return
        if not self.args.use_relation_aware_hnm:
            return
        if self.logger is not None and self.args.model_name_save:
            self.logger.info(
                f"RelationAwareHNMWarmstart | checkpoint={self.args.model_name_save} lr={self.args.lr}"
            )
    ...
    def _optim_init_type_modality_bias_only(self, opt, accumulation_step=None):
        ...
        if bias is None and cdmr is None and dehr is None and tmhg is None:
        ...
        if self.rank == 0:
            ...
            self.logger.info(
                ...
                f"use_cdmr={getattr(opt, 'use_cdmr_router', False)} "
                f"use_dehr={getattr(opt, 'use_dehr_router', False)} "
                f"use_tmhg={getattr(opt, 'use_tmhg_router', False)} weight_decay=0.0"
            )
    ...
    def _log_type_modality_bias_table(self):
        if self.rank != 0 or not (getattr(self.args, "use_type_modality_bias", False) or getattr(self.args, "use_dehr_router", False) or getattr(self.args, "use_tmhg_router", False)):
            return
        ...
        if cdmr is not None:
            ...
            parts = []
            if beta is not None:
                parts.append("beta=" + ",".join(f"{float(x):.4f}" for x in beta.detach().cpu().tolist()))
            if residual is not None:
                res = residual.detach().cpu()
                parts.append(f"res_abs_mean={float(res.abs().mean()):.4f}")
                parts.append(f"res_abs_max={float(res.abs().max()):.4f}")
            if entropy is not None:
                ent = entropy.detach().cpu()
                parts.append(f"entropy_mean={float(ent.mean()):.4f}")
            if parts:
                self.logger.info("CDMRRouterTable | epoch={} ".format(self.epoch) + " ".join(parts))
        dehr = getattr(fusion, "dehr_router", None)
        if dehr is not None:
            parts = []
        ...
        if tmhg is not None:
            try:
                if getattr(tmhg, "input_mode", "stat") == "semantic":
                    q = getattr(tmhg, "last_q", None)
                    type_ids = getattr(tmhg, "last_type_ids", None)
                    residual = getattr(tmhg, "last_residual", None)
                    evidence = getattr(tmhg, "last_evidence", None)
                    gamma = getattr(tmhg, "last_gamma", None)
                    token_norm = getattr(tmhg, "last_token_norm", None)
                    if q is None or type_ids is None:
                        raise ValueError("semantic TMHG has no cached q/type_ids")
                    q = q.detach().cpu()
                    type_ids = type_ids.detach().cpu()
                    rows = []
                    for idx in sorted(type_ids.unique().tolist()):
                        mask = type_ids == idx
                        vals = q[mask].mean(dim=0)
                        type_name = type_names[idx] if idx < len(type_names) else str(idx)
                        values = " ".join(
                            f"{modal}={float(vals[j]):.4f}"
                            for j, modal in enumerate(modal_names[:vals.numel()])
                    ...
                    self.logger.info(
                        "TMHGTable | "
                        f"epoch={self.epoch} mode=semantic "
                        f"residual_scale={float(getattr(self.args, 'tmhg_residual_scale', 0.25)):.4f} "
                        f"evidence_abs_mean={float(evidence.detach().abs().mean().item()) if torch.is_tensor(evidence) else 0.0:.4f} "
                        f"residual_abs_mean={float(residual.detach().abs().mean().item()) if torch.is_tensor(residual) else 0.0:.4f} "
                        f"gamma_mean={float(gamma.detach().mean().item()) if torch.is_tensor(gamma) else 0.0:.4f} "
                        f"gamma_max={float(gamma.detach().max().item()) if torch.is_tensor(gamma) else 0.0:.4f} "
                        f"token_norm_mean={float(token_norm.detach().mean().item()) if torch.is_tensor(token_norm) else 0.0:.4f} "
                        + " || ".join(rows)
                    )
                else:
                    device = next(tmhg.parameters()).device
                    logits, evidence, residual, _alpha, strength = tmhg._compute_type_logits(device)
                    q = torch.softmax(logits.detach().cpu(), dim=-1)
                    rows = []
                    for idx in range(q.shape[0]):
                        type_name = type_names[idx] if idx < len(type_names) else str(idx)
                        values = " ".join(
                            f"{modal}={float(q[idx, j]):.4f}"
                            for j, modal in enumerate(modal_names[:q.shape[1]])
                        )
                        rows.append(f"{type_name}[{idx}] {values}")
                    last_mix = getattr(tmhg, "last_global_mix", None)
                    if torch.is_tensor(last_mix):
                        mix_value = float(last_mix.detach().cpu().item())
                    else:
                        mix_value = 0.0
                    self.logger.info(
                        "TMHGTable | "
                        f"epoch={self.epoch} gamma={float(getattr(self.args, 'tmhg_gamma', 1.0)):.4f} "
                        f"residual_scale={float(getattr(self.args, 'tmhg_residual_scale', 0.25)):.4f} "
                        f"global_mix={mix_value:.4f} "
                        f"evidence_abs_mean={float(evidence.detach().abs().mean().item()):.4f} "
                        f"residual_abs_mean={float(residual.detach().abs().mean().item()):.4f} "
                        f"strength={','.join(f'{float(x):.4f}' for x in strength.detach().cpu().squeeze(-1).tolist())} "
                        f"prior_kl={float(getattr(tmhg, 'last_prior_kl').detach().cpu().item()) if torch.is_tensor(getattr(tmhg, 'last_prior_kl', None)) else 0.0:.4f} "
                        f"prior_cos={float(getattr(tmhg, 'last_prior_cos').detach().cpu().item()) if torch.is_tensor(getattr(tmhg, 'last_prior_cos', None)) else 0.0:.4f} "
                        + " || ".join(rows)
                    )
            except Exception as exc:
                self.logger.info(f"TMHGTable | skipped error={exc}")
    ...
    def run(self):
        ...
        self.curr_proto_stats = defaultdict(float)
        self.curr_proto_stats_count = 0
        self.curr_ocmf_stats = defaultdict(float)
        self.curr_ocmf_stats_count = 0
        self.curr_ogar_stats = defaultdict(float)
        self.curr_ogar_stats_count = 0
        ...
        self.tmhg_calibration_pretrain()
    ...
    def _tmhg_calib_router_params(self, router):
        return [p for p in router.parameters() if p.requires_grad]
    ...
    def _tmhg_calib_compose_embedding(self, modal_embs, weight_norm, router, type_ids):
        bias = router(weight_norm, entity_type_ids=type_ids, modal_embs=modal_embs)
        self._tmhg_calib_last_bias = bias
        logits = torch.log(weight_norm.detach().clamp_min(1e-12)) + bias
        tau_route = float(getattr(self.args, "cdmr_tau_route", 1.0))
        tau_route = max(tau_route, 1e-6)
        weight_final = F.softmax(logits / tau_route, dim=-1)
    ...
    def _tmhg_calib_hard_modal_loss(self, emb, modal_embs, links, left_bank, right_bank, left_pos, right_pos, tau, batch_size):
        ...
        links = links[perm]
        k = max(int(getattr(self.args, "tmhg_calib_hard_topk", 16)), 1)
        losses = []
        ...
        for start in range(0, total, batch_size):
            ...
            for modal_emb in modal_embs:
                modal_emb = F.normalize(modal_emb.detach(), dim=1)
                if valid_lr.any():
                    sim_lr = modal_emb[left[valid_lr]] @ modal_emb[right_bank].t()
                    row = torch.arange(sim_lr.shape[0], device=emb.device)
                    sim_lr = sim_lr.clone()
                    sim_lr[row, target_r[valid_lr]] = -1e9
                    hard = sim_lr.topk(k=min(k, sim_lr.shape[1]), dim=1).indices
                    cand_ids = torch.cat([right[valid_lr].unsqueeze(1), right_bank[hard]], dim=1)
                    sim_joint = emb[left[valid_lr]].unsqueeze(1) * emb[cand_ids]
                    logits = sim_joint.sum(dim=-1) / tau
                    local_losses.append(F.cross_entropy(logits, torch.zeros(logits.shape[0], dtype=torch.long, device=emb.device)))
                if valid_rl.any():
                    sim_rl = modal_emb[right[valid_rl]] @ modal_emb[left_bank].t()
                    row = torch.arange(sim_rl.shape[0], device=emb.device)
                    sim_rl = sim_rl.clone()
                    sim_rl[row, target_l[valid_rl]] = -1e9
                    hard = sim_rl.topk(k=min(k, sim_rl.shape[1]), dim=1).indices
                    cand_ids = torch.cat([left[valid_rl].unsqueeze(1), left_bank[hard]], dim=1)
                    sim_joint = emb[right[valid_rl]].unsqueeze(1) * emb[cand_ids]
                    logits = sim_joint.sum(dim=-1) / tau
                    local_losses.append(F.cross_entropy(logits, torch.zeros(logits.shape[0], dtype=torch.long, device=emb.device)))
            if local_losses:
    ...
    def _tmhg_modal_utility_target(self, modal_embs, links, left_bank, right_bank, left_pos, right_pos, batch_size):
        if not torch.is_tensor(links):
            links = torch.as_tensor(links, dtype=torch.long, device=modal_embs[0].device)
        total = int(links.shape[0])
        utility_chunks = []
        left_chunks = []
        for start in range(0, total, batch_size):
            ...
            for modal_emb in modal_embs:
                modal_emb = F.normalize(modal_emb.detach(), dim=1)
                sim = modal_emb[left[valid_lr]] @ modal_emb[right_bank].t()
                pos = sim.gather(1, target_r[valid_lr].unsqueeze(1)).squeeze(1)
                sim = sim.clone()
                row = torch.arange(sim.shape[0], device=sim.device)
                sim[row, target_r[valid_lr]] = -1e9
                hard = sim.max(dim=1).values
                modal_scores.append((pos - hard).unsqueeze(1))
            utility_chunks.append(torch.cat(modal_scores, dim=1))
            left_chunks.append(left[valid_lr])
        if not utility_chunks:
            return None, None
        utility = torch.cat(utility_chunks, dim=0)
        left_ids = torch.cat(left_chunks, dim=0)
        temp = max(float(getattr(self.args, "tmhg_calib_modal_target_temp", 0.5)), 1e-6)
        return F.softmax(utility / temp, dim=-1), left_ids

    def _tmhg_type_balanced_modal_target(self, modal_embs, links, left_bank, right_bank, left_pos, right_pos, batch_size, type_ids):
        utility_target, left_ids = self._tmhg_modal_utility_target(
            modal_embs, links, left_bank, right_bank, left_pos, right_pos, batch_size
        )
        if utility_target is None or left_ids is None or type_ids is None:
            return None, None
        left_type = type_ids[left_ids].long()
        type_count = int(left_type.max().item()) + 1 if left_type.numel() > 0 else 0
        if type_count <= 0:
            return None, None
        modal_num = utility_target.shape[-1]
        type_sum = utility_target.new_zeros((type_count, modal_num))
        type_cnt = utility_target.new_zeros((type_count, 1))
        type_sum.index_add_(0, left_type, utility_target)
        type_cnt.index_add_(0, left_type, utility_target.new_ones((utility_target.shape[0], 1)))
        type_avg = type_sum / type_cnt.clamp_min(1.0)
        temp = max(float(getattr(self.args, "tmhg_calib_type_balance_temp", 0.5)), 1e-6)
        return F.softmax(torch.log(type_avg.clamp_min(1e-12)) / temp, dim=-1), left_type
    ...
    def tmhg_calibration_pretrain(self):
        epochs = int(getattr(self.args, "tmhg_calib_pretrain_epochs", 0))
        if epochs <= 0 or not getattr(self.args, "use_tmhg_router", False):
            return
        fusion = getattr(self.model.multimodal_encoder, "fusion", None)
        router = getattr(fusion, "tmhg_router", None)
        if router is None or self.rank != 0:
            return
        train_links = np.asarray(self.train_ill, dtype=np.int64)
        if train_links.ndim != 2 or train_links.shape[0] < 4:
            return
        ...
        with torch.no_grad():
            ...
            modal_embs = getattr(fusion, "last_modal_embs", None)
            if modal_embs is None:
                raise ValueError("TMHG calibration requires cached modal embeddings from fusion.")
            modal_embs = [emb.detach() for emb in modal_embs[:router.modal_num]]
            weight_norm = weight_norm.detach()
            type_ids = self.model.entity_type_ids.detach()

        rng = np.random.default_rng(int(getattr(self.args, "random_seed", 42)))
        perm = rng.permutation(train_links.shape[0])
        val_ratio = min(max(float(getattr(self.args, "tmhg_calib_pretrain_val_ratio", 0.2)), 0.05), 0.5)
        val_count = max(1, int(round(train_links.shape[0] * val_ratio)))
        calib_val = train_links[perm[:val_count]]
        calib_train = train_links[perm[val_count:]]
        if calib_train.shape[0] == 0:
            calib_train = calib_val
        ...
        for p in router.parameters():
            p.requires_grad = True
        params = self._tmhg_calib_router_params(router)
        optimizer = torch.optim.AdamW(
            params,
            lr=float(getattr(self.args, "tmhg_calib_pretrain_lr", 1e-3)),
            weight_decay=float(getattr(self.args, "tmhg_calib_pretrain_weight_decay", 1e-4)),
        )
        tau = max(float(getattr(self.args, "tmhg_calib_pretrain_tau", 0.05)), 1e-6)
        batch_size = max(int(getattr(self.args, "tmhg_calib_pretrain_batch_size", 512)), 1)
        patience = max(int(getattr(self.args, "tmhg_calib_pretrain_patience", 8)), 1)
        bias_l2 = max(float(getattr(self.args, "tmhg_calib_bias_l2", 0.0)), 0.0)
        best_metric = -1.0
        best_state = copy.deepcopy(router.state_dict())
        best_loss = float("inf")
        final_state = None
        ...
        self.logger.info(
            "TMHGCalibPretrain | "
            f"epochs={epochs} train={calib_train.shape[0]} val={calib_val.shape[0]} "
            f"lr={float(getattr(self.args, 'tmhg_calib_pretrain_lr', 1e-3))} "
            f"tau={tau} batch={batch_size} val_ratio={val_ratio:.3f} bias_l2={bias_l2}"
        )
        selection = getattr(self.args, "tmhg_calib_selection", "best_val")
        for ep in range(epochs):
            router.train()
            emb = self._tmhg_calib_compose_embedding(modal_embs, weight_norm, router, type_ids)
            if getattr(self.args, "tmhg_calib_loss", "hard_modal") == "hard_modal":
                loss = self._tmhg_calib_hard_modal_loss(
                    emb,
                    modal_embs,
                    calib_train,
                    candidate_left,
            ...
            if loss is None:
                break
            joint_weight = float(getattr(self.args, "tmhg_calib_joint_weight", 1.0))
            loss = joint_weight * loss
            modal_utility_weight = float(getattr(self.args, "tmhg_calib_modal_utility_weight", 0.0))
            if modal_utility_weight > 0 and getattr(router, "input_mode", "stat") == "semantic":
                utility_target, utility_left_ids = self._tmhg_modal_utility_target(
                    modal_embs,
                    calib_train,
                    candidate_left,
                    candidate_right,
                ...
                if utility_target is not None and utility_left_ids is not None:
                    q_live = getattr(router, "_last_q_all_live", None)
                    if q_live is not None:
                        aux_q = q_live[utility_left_ids].clamp_min(1e-12)
                        aux_loss = -(utility_target * torch.log(aux_q)).sum(dim=-1).mean()
                        loss = loss + modal_utility_weight * aux_loss
            type_balance_weight = float(getattr(self.args, "tmhg_calib_type_balance_weight", 0.0))
            if type_balance_weight > 0 and getattr(router, "input_mode", "stat") == "semantic":
                type_target, target_type_ids = self._tmhg_type_balanced_modal_target(
                    modal_embs,
                    calib_train,
                    candidate_left,
                    candidate_right,
                ...
                q_live = getattr(router, "_last_q_all_live", None)
                if type_target is not None and target_type_ids is not None and q_live is not None:
                    q_type = []
                    type_bank = []
                    for tid in torch.unique(target_type_ids):
                        mask = (type_ids == tid)
                        if mask.any():
                            q_type.append(q_live[mask].mean(dim=0, keepdim=True))
                            type_bank.append(tid.view(1))
                    if q_type and type_bank:
                        q_type = torch.cat(q_type, dim=0).clamp_min(1e-12)
                        type_bank = torch.cat(type_bank, dim=0)
                        max_tid = min(type_target.shape[0], int(type_bank.max().item()) + 1)
                        valid = type_bank < max_tid
                        if valid.any():
                            aligned_q = q_type[valid]
                            aligned_t = type_target[type_bank[valid]]
                            type_loss = -(aligned_t * torch.log(aligned_q)).sum(dim=-1).mean()
                            loss = loss + type_balance_weight * type_loss
            if bias_l2 > 0 and getattr(self, "_tmhg_calib_last_bias", None) is not None:
                loss = loss + bias_l2 * self._tmhg_calib_last_bias.pow(2).mean()
            optimizer.zero_grad(set_to_none=True)
            loss.backward()
            torch.nn.utils.clip_grad_norm_(params, float(getattr(self.args, "clip", 1.0)))
            optimizer.step()
            ...
            router.eval()
            with torch.no_grad():
                emb_eval = self._tmhg_calib_compose_embedding(modal_embs, weight_norm, router, type_ids)
                val_l, val_r, val_avg = self._dehr_calib_hits1(
                    emb_eval,
                    calib_val,
                    candidate_left,
            ...
            if selection == "best_val" and stale >= patience:
                self.logger.info(
                    f"TMHGCalibPretrain | early_stop epoch={ep} best_val_avg={best_metric:.4f}"
                )
                break
        ...
        self.logger.info(
            f"TMHGCalibPretrain | loaded_{selection} val_avg={val_avg:.4f} val_l2r={val_l:.4f} val_r2l={val_r:.4f}"
        )
        self._log_type_modality_bias_table()
        for p, requires_grad in previous_requires_grad.items():
            p.requires_grad = requires_grad
    ...
    def il_for_ea(self):
        with torch.no_grad():
            if self.args.model_name in ["SGMEA"]:
    ...
    def _evaluate_distance_matrix(self, distance_matrix):
        top_k = [1, 10, 50]
        ...
        for idx in range(distance_matrix.shape[0]):
            _, indices = torch.sort(distance_matrix[idx, :], descending=False)
        ...
        for idx in range(distance_matrix.shape[1]):
            _, indices = torch.sort(distance_matrix[:, idx], descending=False)
            rank = (indices == idx).nonzero(as_tuple=False).squeeze().item()
            mean_r2l += (rank + 1)
            mrr_r2l += 1.0 / (rank + 1)
        ...
        mean_l2r /= distance_matrix.shape[0]
        mean_r2l /= distance_matrix.shape[1]
        mrr_l2r /= distance_matrix.shape[0]
        mrr_r2l /= distance_matrix.shape[1]
        for i in range(len(top_k)):
            acc_l2r[i] = round(acc_l2r[i] / distance_matrix.shape[0], 4)
            acc_r2l[i] = round(acc_r2l[i] / distance_matrix.shape[1], 4)
    ...
    def _build_m2dsa_entropy_gate(self, prob_src, prob_tgt):
        proto_dim = prob_src.shape[1]
        if proto_dim <= 1:
            return prob_src.new_ones((prob_src.shape[0], prob_tgt.shape[0]))
        max_entropy = float(np.log(proto_dim))
        entropy_src = -torch.sum(prob_src * torch.log(prob_src.clamp_min(1e-12)), dim=1)
        entropy_tgt = -torch.sum(prob_tgt * torch.log(prob_tgt.clamp_min(1e-12)), dim=1)
        gate_src = (1.0 - entropy_src / max_entropy).clamp_(0.0, 1.0)
        gate_tgt = (1.0 - entropy_tgt / max_entropy).clamp_(0.0, 1.0)
        return torch.sqrt(gate_src.unsqueeze(1) * gate_tgt.unsqueeze(0))

    def _build_type_only_probs(self, entity_ids):
        type_ids = self.model.entity_type_ids[entity_ids]
        type_count = int(self.model.top_type_count)
        probs = F.one_hot(type_ids, num_classes=type_count).float()
        return probs

    def _run_m2dsa_probe_sweep(self, distance_matrix, all_probs, test_left, test_right):
        prob_src = all_probs[test_left]
        prob_tgt = all_probs[test_right]
        beta_values = [float(beta.strip()) for beta in self.args.m2dsa_betas.split(",") if beta.strip()]
        beta_global_values = [float(beta.strip()) for beta in self.args.m2dsa_beta_globals.split(",") if beta.strip()]
        variants = [variant.strip() for variant in self.args.m2dsa_variants.split(",") if variant.strip()]

        proto_dim = prob_src.shape[1]
        specific_dim = min(self.args.m2dsa_specific_dim, proto_dim)
        sim_P = torch.mm(prob_src, prob_tgt.t())
        sim_P_specific = torch.mm(prob_src[:, :specific_dim], prob_tgt[:, :specific_dim].t())
        sim_P_global = None
        if specific_dim < proto_dim:
            sim_P_global = torch.mm(prob_src[:, specific_dim:], prob_tgt[:, specific_dim:].t())
        gate = self._build_m2dsa_entropy_gate(prob_src, prob_tgt)

        results = []
        for variant in variants:
            if variant == "plain":
                for beta in beta_values:
                    distance_final = distance_matrix - beta * sim_P
                    probe_result = self._evaluate_distance_matrix(distance_final)
                    probe_result.update({"variant": variant, "beta": beta, "beta_g": None})
                    results.append(probe_result)
            elif variant == "specific_only":
                for beta in beta_values:
                    distance_final = distance_matrix - beta * sim_P_specific
                    probe_result = self._evaluate_distance_matrix(distance_final)
                    probe_result.update({"variant": variant, "beta": beta, "beta_g": None})
                    results.append(probe_result)
            elif variant == "confidence_gated":
                for beta in beta_values:
                    distance_final = distance_matrix - beta * gate * sim_P
                    probe_result = self._evaluate_distance_matrix(distance_final)
                    probe_result.update({
                        "variant": variant,
                        "beta": beta,
                    ...
                    results.append(probe_result)
            elif variant == "specific_global" and sim_P_global is not None:
                for beta in beta_values:
                    for beta_g in beta_global_values:
                        distance_final = distance_matrix - beta * sim_P_specific - beta_g * sim_P_global
                        probe_result = self._evaluate_distance_matrix(distance_final)
                        probe_result.update({"variant": variant, "beta": beta, "beta_g": beta_g})
                        results.append(probe_result)
        return results
    ...
    def train(self, _tqdm):
        ...
        for batch in self.train_dataloader:
            ...
            if self.step % accumulation_steps == 0:
                ...
                clip_norm = self.args.relation_hnm_grad_clip if self.args.use_relation_aware_hnm else self.args.clip
    ...
    def output_statistic(self, loss, output):
        ...
        if 'proto_stats' in output and output['proto_stats'] is not None:
            for key, value in output['proto_stats'].items():
                self.curr_proto_stats[key] += float(value)
            self.curr_proto_stats_count += 1
        if 'ocmf_stats' in output and output['ocmf_stats'] is not None:
            for key, value in output['ocmf_stats'].items():
                self.curr_ocmf_stats[key] += float(value)
            self.curr_ocmf_stats_count += 1
        if 'ogar_stats' in output and output['ogar_stats'] is not None:
            for key, value in output['ogar_stats'].items():
                self.curr_ogar_stats[key] += float(value)
            self.curr_ogar_stats_count += 1
    ...
    def update_loss_log(self):
        ...
        relation_log_line = None
        relation_keys = [
            "loss_original_raw",
            "batch_hit_anchor_count",
            "shield_mask_hit_count",
            "safe_weight_hit_count",
            "close_weight_hit_count",
            "safe_denominator_weight",
            "close_denominator_weight",
            "positive_mask_violation_count",
            "positive_weight_violation_max_abs",
            "mask_weight_overlap_count",
            "cross_weight_nonunit_count",
            "cross_weight_min",
            "cross_weight_max",
            "cross_weight_mean",
            ...
            "cdmr_beta_mean",
            "cdmr_beta_max",
            "cdmr_residual_abs_mean",
            "cdmr_residual_abs_max",
            "cdmr_entropy_mean",
            "cdmr_pairwise_cos_mean",
            "dehr_global_bias_abs_mean",
            "dehr_type_bias_abs_mean",
            "dehr_evidence_bias_abs_mean",
            "tmhg_bias_abs_mean",
            ...
            "tmhg_weight_max",
            "tmhg_residual_abs_mean",
            "tmhg_evidence_abs_mean",
            "tmhg_global_mix",
            "tmhg_prior_kl",
            "tmhg_prior_cos",
            "tmhg_type_strength_mean",
            "tmhg_type_strength_max",
        ]
        relation_items = []
        for key in relation_keys:
            if key in avg_loss_dic:
                relation_items.append(f"{key}={avg_loss_dic[key]:.4f}")
        if relation_items:
        ...
        if self.curr_proto_stats_count > 0:
            proto_vis_dict = {}
            for key, value in self.curr_proto_stats.items():
                proto_vis_dict[key] = value / self.curr_proto_stats_count
            if self.writer is not None:
                self.writer.add_scalars("prototype", proto_vis_dict, self.step)
            proto_items = [f"{k}={v:.4f}" for k, v in sorted(proto_vis_dict.items())]
            proto_log_line = "Proto | " + " ".join(proto_items)

        ocmf_log_line = None
        if self.curr_ocmf_stats_count > 0:
            ocmf_vis_dict = {}
            for key, value in self.curr_ocmf_stats.items():
                ocmf_vis_dict[key] = value / self.curr_ocmf_stats_count
            if self.writer is not None:
                self.writer.add_scalars("ocmf", ocmf_vis_dict, self.step)
            ocmf_items = [f"{k}={v:.4f}" for k, v in sorted(ocmf_vis_dict.items())]
            ocmf_log_line = "OCMF | " + " ".join(ocmf_items)

        ogar_log_line = None
        if self.curr_ogar_stats_count > 0:
            ogar_vis_dict = {}
            for key, value in self.curr_ogar_stats.items():
                ogar_vis_dict[key] = value / self.curr_ogar_stats_count
            if self.writer is not None:
                self.writer.add_scalars("ogar", ogar_vis_dict, self.step)
            ogar_items = [f"{k}={v:.4f}" for k, v in sorted(ogar_vis_dict.items())]
            ogar_log_line = "OGAR | " + " ".join(ogar_items)
        ...
        self.curr_loss_dic_count = 0
        for key in self.curr_proto_stats:
            self.curr_proto_stats[key] = 0.
        self.curr_proto_stats_count = 0
        for key in self.curr_ocmf_stats:
            self.curr_ocmf_stats[key] = 0.
        self.curr_ocmf_stats_count = 0
        for key in self.curr_ogar_stats:
            self.curr_ogar_stats[key] = 0.
        self.curr_ogar_stats_count = 0
        if proto_log_line is not None and self.rank == 0:
            self.logger.info(proto_log_line)
        if ocmf_log_line is not None and self.rank == 0:
            self.logger.info(ocmf_log_line)
        if ogar_log_line is not None and self.rank == 0:
    ...
    def _test(self, test_left, test_right, last_epoch=False, save_name="", loss=None):
        with torch.no_grad():
            ...
            if (
                self.args.model_name in ["SGMEA"]
                and self.args.use_type_prototypes
                and self.model.prototype_head is not None
                and run_geometry_probe is not None
            ):
                _, _, _, _, _, _, _, _, _, _, _, raw_joint_emb, _, _ = self.model.multimodal_encoder(
                    self.model.input_idx,
                    self.model.adj,
                    self.model.img_features,
                    self.model.rel_features,
                    self.model.att_features,
                    self.model.name_features,
                    self.model.char_features,
                )
                _, all_probs = self.model.prototype_head(raw_joint_emb, self.model.entity_type_ids)
                geometry_probe_stats = run_geometry_probe(
                    final_emb[test_left],
                    final_emb[test_right],
                    all_probs[test_left],
                    all_probs[test_right],
                )
        ...
        sim_matrix = torch.mm(final_emb[test_left], final_emb[test_right].t())
        m2dsa_probe_results = []
        if self.args.use_m2dsa_probe and all_probs is None:
            all_probs = self._build_type_only_probs(self.model.input_idx)

        if self.args.use_m2dsa_probe and all_probs is not None:
            m2dsa_probe_results = self._run_m2dsa_probe_sweep(
        ...
        loss_upper_bound_stats = None
        if all_probs is not None and run_loss_upper_bound_probe is not None:
            loss_upper_bound_stats = run_loss_upper_bound_probe(
                sim_matrix=sim_matrix,
                prob_src=all_probs[test_left],
                prob_tgt=all_probs[test_right],
                hits1_percent=acc_l2r[0] * 100.0,
            )
        gc.collect()
        ...
        if self.rank == 0:
            ...
            if loss_upper_bound_stats is not None:
                upper_line = (
                    "LossUpperBoundProbe | "
                    f"total_errors={loss_upper_bound_stats['loss_upper_total_errors']} "
                    f"intra_errors={loss_upper_bound_stats['loss_upper_intra_errors']} "
                    f"inter_errors={loss_upper_bound_stats['loss_upper_inter_errors']} "
                    f"intra_error_ratio={loss_upper_bound_stats['loss_upper_intra_error_ratio']:.4f} "
                    f"ceiling_hits1={loss_upper_bound_stats['loss_upper_ceiling_hits1']:.4f}"
                )
                self.logger.info(upper_line)
                if self.writer is not None:
                    upper_vis_dict = {
                        "intra_error_ratio": loss_upper_bound_stats["loss_upper_intra_error_ratio"],
                        "ceiling_hits1": loss_upper_bound_stats["loss_upper_ceiling_hits1"],
                    }
                    self.writer.add_scalars("loss_upper_bound_probe", upper_vis_dict, self.step)
            if m2dsa_probe_results:
                best_by_variant = {}
                for probe_result in m2dsa_probe_results:
                    avg_hits1 = 0.5 * (probe_result["acc_l2r"][0] + probe_result["acc_r2l"][0])
                    avg_mrr = 0.5 * (probe_result["mrr_l2r"] + probe_result["mrr_r2l"])
                    probe_result["avg_hits1"] = avg_hits1
                    probe_result["avg_mrr"] = avg_mrr
                    current_best = best_by_variant.get(probe_result["variant"])
                    if current_best is None or avg_hits1 > current_best["avg_hits1"]:
                        best_by_variant[probe_result["variant"]] = probe_result
                    m2dsa_line = (
                        "M2DSAProbe | "
                        f"variant={probe_result['variant']} "
                        f"beta={probe_result['beta']:.4f} "
                        f"beta_g={probe_result['beta_g'] if probe_result['beta_g'] is not None else 'NA'} "
                        f"hits1_l2r={probe_result['acc_l2r'][0]:.4f} "
                        f"hits1_r2l={probe_result['acc_r2l'][0]:.4f} "
                        f"mrr_l2r={probe_result['mrr_l2r']:.4f} "
                        f"mrr_r2l={probe_result['mrr_r2l']:.4f}"
                    )
                    if "gate_mean" in probe_result:
                        m2dsa_line += f" gate_mean={probe_result['gate_mean']:.4f}"
                    self.logger.info(m2dsa_line)
                    if self.writer is not None:
                        self.writer.add_scalars(
                            "m2dsa_probe_hits1",
                            {
                                f"{probe_result['variant']}_b_{probe_result['beta']:.2f}" +
                                (f"_bg_{probe_result['beta_g']:.2f}" if probe_result['beta_g'] is not None else ""):
                                probe_result["acc_l2r"][0]
                            },
                            self.step,
                        )
                for variant, probe_result in sorted(best_by_variant.items()):
                    summary_line = (
                        "M2DSASummary | "
                        f"variant={variant} "
                        f"best_beta={probe_result['beta']:.4f} "
                        f"best_beta_g={probe_result['beta_g'] if probe_result['beta_g'] is not None else 'NA'} "
                        f"best_hits1_avg={probe_result['avg_hits1']:.4f} "
                        f"best_mrr_avg={probe_result['avg_mrr']:.4f}"
                    )
                    self.logger.info(summary_line)
            if geometry_probe_stats is not None:
                probe_line = (
                    "GeometryProbe | "
                    f"gt_sim={geometry_probe_stats['geometry_probe_gt_sim']:.4f} "
                    f"intra_sim={geometry_probe_stats['geometry_probe_intra_sim']:.4f} "
                    f"inter_sim={geometry_probe_stats['geometry_probe_inter_sim']:.4f} "
                    f"margin={geometry_probe_stats['geometry_probe_margin']:.4f} "
                    f"intra_pairs={geometry_probe_stats['geometry_probe_intra_pair_count']} "
                    f"inter_pairs={geometry_probe_stats['geometry_probe_inter_pair_count']}"
                )
                self.logger.info(probe_line)
                if self.writer is not None:
                    probe_vis_dict = {
                        "gt_sim": geometry_probe_stats["geometry_probe_gt_sim"],
                        "intra_sim": geometry_probe_stats["geometry_probe_intra_sim"],
                        "inter_sim": geometry_probe_stats["geometry_probe_inter_sim"],
                        "margin": geometry_probe_stats["geometry_probe_margin"],
                    }
                    self.writer.add_scalars("geometry_probe", probe_vis_dict, self.step)
            self.early_stop_count -= 1
