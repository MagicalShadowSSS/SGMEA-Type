...
FB_TYPE_TO_ID = {name: idx for idx, name in enumerate(FB_TYPE_NAMES)}
...
def load_eva_data(logger, args):
    ...
    entity_type_ids, entity_is_generic, top_type_names, top_type_count = load_entity_types(
        logger=logger,
        args=args,
        ent_num=ENT_NUM,
    )
    ...
    Numericals = [(ent2id_dict[i[0]], db_str(i[1]), db_time(i[2])) for i in Numericals]
    ...
    for id_, attr_values in id_to_attributes.items():
        if attr_values:
            attr_value_strs = [f'{attr}: {value}' for attr, value in attr_values]
            id_to_str[id_] = ', '.join(attr_value_strs)
        else:
            id_to_str[id_] = ''


    from sentence_transformers import SentenceTransformer
    labse_path = os.environ.get("LABSE_PATH", "/gly/tongqiang/dongyufeng/models/LaBSE")
    model = SentenceTransformer(labse_path, local_files_only=True).cuda()


    outputs = []
    texts = [id_to_str[id_] for id_ in range(num_entities)]
    batch_size = 2048
    sent_batches = [texts[i:i + batch_size] for i in range(0, len(texts), batch_size)]
    ...
    return {
        ...
        'input_idx': input_idx,
        'adj': adj,
        'left_entity_ids': torch.LongTensor(left_ents),
        'right_entity_ids': torch.LongTensor(right_ents),
        'entity_type_ids': torch.LongTensor(entity_type_ids),
        'entity_is_generic': torch.BoolTensor(entity_is_generic),
...
def load_entity_types(logger, args, ent_num):
    type_names = FB_TYPE_NAMES
    type_count = len(type_names)
    entity_type_ids = np.full(ent_num, FB_TYPE_TO_ID["Entity"], dtype=np.int64)
    entity_is_generic = np.ones(ent_num, dtype=np.bool_)

    if not getattr(args, "external_anchor_type_jsonl", ""):
        logger.info("entity coarse types: no external type jsonl, all entities mapped to Entity")
        return entity_type_ids, entity_is_generic, type_names, type_count

    if not osp.exists(args.external_anchor_type_jsonl):
        raise FileNotFoundError(f"external anchor type jsonl not found: {args.external_anchor_type_jsonl}")

    with open(args.external_anchor_type_jsonl, "r", encoding="utf-8") as f:
        for line in f:
            line = line.strip()
            if not line:
                continue
            row = json.loads(line)
            ent_id = int(row["id"])
            coarse_type = row.get("coarse_type", "Entity")
            type_id = FB_TYPE_TO_ID.get(coarse_type, FB_TYPE_TO_ID["Entity"])
            if 0 <= ent_id < ent_num:
                entity_type_ids[ent_id] = type_id
                entity_is_generic[ent_id] = (type_id == FB_TYPE_TO_ID["Entity"])

    type_counter = Counter(type_names[type_id] for type_id in entity_type_ids.tolist())
    logger.info("entity coarse types: external distribution loaded")
    for name in type_names:
        count = type_counter.get(name, 0)
        logger.info(f"  {name}: {count} ({100.0 * count / ent_num:.2f}%)")
    return entity_type_ids, entity_is_generic, type_names, type_count
