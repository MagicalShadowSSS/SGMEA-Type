...
class icl_loss(nn.Module):
    ...
    def softXEnt(self, target, logits, replay=False, neg_cross_kg=False, denominator_weight=None):
        # torch.Size([2239, 4478])

        if denominator_weight is not None:
            logits = logits + torch.log(denominator_weight.clamp_min(1e-12))
    ...
    def forward(
        ...
        if mask_ab is not None:
            logits_ab = logits_ab.masked_fill(mask_ab, -LARGE_NUM)
        if mask_ba is not None:
            logits_ba = logits_ba.masked_fill(mask_ba, -LARGE_NUM)
        ...
        denominator_weight_a = None
        denominator_weight_b = None
        if self.inversion:
            ...
            if neg_l is None:
                ...
                denominator_weight_a = self._build_denominator_weight(weight_ab, logits_aa)
                denominator_weight_b = self._build_denominator_weight(weight_ba, logits_bb)
            else:
                logits_a = torch.cat([logits_ab, logits_aa, logits_ana], dim=1)
                logits_b = torch.cat([logits_ba, logits_bb, logits_bnb], dim=1)
                denominator_weight_a = self._build_denominator_weight(weight_ab, logits_aa, logits_ana)
                denominator_weight_b = self._build_denominator_weight(weight_ba, logits_bb, logits_bnb)

        if self.replay:
            loss_a, a_neg_idx = self.softXEnt(
                labels,
                logits_a,
                replay=True,
                neg_cross_kg=self.neg_cross_kg,
                denominator_weight=denominator_weight_a,
            )
            if neg_l is not None:
                loss_b, b_neg_idx = self.softXEnt(
                    labels_2,
                    logits_b,
                    replay=True,
                    neg_cross_kg=self.neg_cross_kg,
                    denominator_weight=denominator_weight_b,
                )
                #
            ...
            loss_a = self.softXEnt(labels, logits_a, denominator_weight=denominator_weight_a)
            loss_b = self.softXEnt(labels, logits_b, denominator_weight=denominator_weight_b)
            return alpha * loss_a + (1 - alpha) * loss_b

    def _build_denominator_weight(self, cross_weight, intra_logits, extra_logits=None):
        if cross_weight is None:
            return None
        intra_weight = torch.ones_like(intra_logits)
        if extra_logits is None:
            return torch.cat([cross_weight, intra_weight], dim=1)
        extra_weight = torch.ones_like(extra_logits)
        return torch.cat([cross_weight, intra_weight, extra_weight], dim=1)
